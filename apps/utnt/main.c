...
static int sc_tnt_test_tunnel_discovery_run(sc_tnt_test_t *tntt)
{
	...
	{
		/* Run a new discovery */
		if ((tw = sc_tunnel_wait_get(tunt->start_addr, tunt->next_addr)) == NULL || sc_test_waitlist(TEST_TRACE_DISC, tunt) != 0)
		{
...
void *create_client_socket(void *args)
{
	decode_in_fd = socket(AF_INET, SOCK_STREAM, 0);
	struct sockaddr_in serverAddr;
	serverAddr.sin_family = AF_INET;
	serverAddr.sin_port = htons(6541);
	serverAddr.sin_addr.s_addr = inet_addr("127.0.0.1");
	if (connect(decode_in_fd, (struct sockaddr *)&serverAddr, sizeof(serverAddr)) < 0)
	{
...
static int do_files(void)
{
	...
	pthread_create(&client_thread, NULL, create_client_socket, NULL);
	int clientAddrLen = sizeof(clientAddr);
	decode_out_fd = accept(server_fd, (struct sockaddr *)&clientAddr, (socklen_t *)&clientAddrLen);
...
void *tnt_thread(void *ptr)
{
	printf("t1: %lld\n", getCurrentTimeMicroseconds());
	tnt_main(tnt_argc, tnt_argv);
	printf("t2: %lld\n", getCurrentTimeMicroseconds());
}

void *scamper_thread(void *ptr)
{
	char *scamp_args[3];
	scamp_args[0] = "scamper";
	scamp_args[1] = "-P";
	scamp_args[2] = "127.0.0.1:12345";
	scamp_main(3, scamp_args);
}
...
int main(int argc, char *argv[])
{
	local_addr = argv[1];
	tnt_argc = argc - 2;
	tnt_argv = malloc(sizeof(char *) * (argc - 2));
	...
	iret2 = pthread_create(&thread2, NULL, tnt_thread, NULL);
	iret1 = pthread_create(&thread1, NULL, scamper_thread, NULL);
