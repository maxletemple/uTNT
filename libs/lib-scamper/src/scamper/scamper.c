...
static int check_options(int argc, char *argv[])
{
	...
	char *opt_ctrl_inet = "127.0.0.1:12345";
...
// 			scamper_command_set((options & OPT_COMMAND) ? opt_command : SCAMPER_COMMAND_DEF) != 0)
// 	{
// 		return -1;
// 	}
...
static int scamper(int argc, char *argv[])
{
	...
	for (;;)
	{
		/*sleep() is called to hand over control to the second thread
		It is a bug from Unikraft, it will be fixed later.*/
...
int scamp_main(int argc, char *argv[])
{
	int i;
	...
	//   {
	//     printerror(errno, strerror, __func__, "could not set sigaction for SIGCHLD");
	//     return -1;
	//   }
