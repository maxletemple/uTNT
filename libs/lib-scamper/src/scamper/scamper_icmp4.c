 ...
 * scamper_icmp4.c
 *
 * $Id: scamper_icmp4.c,v 1.114 2016/08/09 08:18:34 mjl Exp $
 *
...
int scamper_icmp4_probe(scamper_probe_t *probe)
{
	...
	// probe->pr_ip_src = scamper_addr_resolve(AF_INET, "192.168.122.2");
	switch (probe->pr_icmp_type)
	{
	case ICMP_ECHO:
	case ICMP_TSTAMP:
...
#if defined(__linux__) || defined(__OpenBSD__) || defined(__sun__) || defined(_WIN32) || defined(__Unikraft__)
